use crate::tree::*;
use abstalg::{UnitaryRing, Polynomials, Domain};
use rand::prelude::*;
    ...
    -> Mask<Poly<S>>
{
    ...
    let polys = assignments.chunks(2)
        .map(|chunk| vec![coefficients[chunk[0]].clone(), coefficients[chunk[1]].clone()])
        .collect();
    Mask::new(size, polys)
}
...
impl NumSubset {
    ...
    fn vec(&self) -> Vec<usize> {
        (0..self.size).into_iter().filter(|&i| self.elts[i]).collect()
    }
}
...
fn place_numbers<R: rand::Rng>(size: usize, rng: &mut R) -> Vec<usize> {
    ...
    loop {
        ...
        for _ in 0..size {
            ...
            for _ in 0..2*size {
                let maybe = order.iter().filter(|&&i| candidates.has(i))
                    .cartesian_product(indices.vec())
                    .filter(|&(&i, x)| cols[x/2].has(i))
                    .filter(|&(&_, x)| (0..2*size).into_iter().filter(
                        |&i| candidates.has(i) && cols[x/2].has(i)).count() == 1
                    )
                    .chain(
                        order.iter().filter(|&&i| candidates.has(i))
                            .cartesian_product(indices.vec())
                            .filter(|&(&i, x)| cols[x/2].has(i))
                            .filter(|&(&i, _)| (0..size).into_iter()
                                    .filter(|&col| (indices.has(2*col) || indices.has(2*col+1))&& cols[col].has(i)
                                    ).count() == 1)                
                    )
                    .chain(
                        order.iter().filter(|&&i| candidates.has(i))
                            .cartesian_product(indices.vec())
                            .filter(|&(&i, x)| cols[x/2].has(i))
                            .min_by_key(|&(&i, x)| (0..2*size).into_iter().filter(
                                |&i| candidates.has(i) && cols[x/2].has(i)
                            ).count()
                                        + (0..size).into_iter().filter(
                                            |&col| (indices.has(2*col) || indices.has(2*col+1)) && cols[col].has(i)
                                        ).count()
                            )
                            .into_iter()
                    )
                ...
                if let Some((&i, x)) = maybe {
                    ...
                    println!("            [");
                    for yp in 0..y {
                    println!("             ; {:?}", &rows[yp*2*size..(yp+1)*2*size])
                }
                    println!("             ; {:?}", row);
                    println!("            ]");
                    println!("w/ order:   {:?}", order.into_iter().filter(|&c| candidates.has(c)).collect::<Vec<usize>>());
                    println!("indices:    {:?}", indices.vec());
                    panic!();
                     */
                }
            }
    ...
    println!("            [");
    for y in 0..size {
        println!("             ; {:?}", &rows[y*2*size..(y+1)*2*size])
    }
    println!("            ]");
     */
    rows
...
type Poly<S: UnitaryRing> = Vec<S::Elem>; 
...
{
    match op {
        Op::Add => Some( polys.into_iter()
                         .fold(Polynomials::zero(ring), |val, p| Polynomials::add(ring, &val, p)) ),
        Op::Mul => Some( polys.into_iter()
                         .fold(Polynomials::one(ring), |val, p| Polynomials::mul(ring, &val, p)) ),
        Op::Sub => if let [ref p0, ref p1] = polys[..] { Some( Polynomials::sub(ring, &p0, &p1) ) } else { None }
    }
}
...
pub fn generate<S: UnitaryRing, R: rand::Rng> (
    size: usize,
    tiles: &HashMap<usize, Vec<Node>>,
    base_ring: S,
    coefficients: &Vec<S::Elem>,
